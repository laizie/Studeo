  ...
  return (
    ...
    >
      ...
      <div className="relative bg-surface rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6">
        ...
        {message && (
          <p className="mt-2 text-sm text-stone-600 dark:text-[#e0b870]">{message}</p>
        )}
        <div className="flex justify-end gap-2 mt-5">
          <button
            ref={cancelRef}
            onClick={onClose}
            className="px-4 py-2 text-sm text-stone-600 dark:text-[#c4a882] hover:text-stone-800 dark:hover:text-[#e8d5c0] rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-stone-400 transition-colors"
          >
            Cancel
          </button>
          <button
