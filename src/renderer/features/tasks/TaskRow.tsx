...
export default function TaskRow({ task, onEdit }: Props) {
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-2.5 group hover:bg-stone-50 rounded-lg transition-colors">
      <button
      ...
      <span className={`flex-1 text-sm truncate ${isCompleted ? 'line-through text-stone-400' : 'text-stone-800'}`}>
        {task.name}
      </span>
